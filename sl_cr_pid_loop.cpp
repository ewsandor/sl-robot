...
{
  const SETPOINT_T error_integrated_temp = error_integrated + this->get_error();
  ...
  if(set_output(new_output))
  {
    /* Only update integral if output is inbounds, otherwise integral can grow unbounded if output is being capped */
    error_integrated = error_integrated_temp;
  }
